...
//                 <div>{resultText}</div>
//                 <Pagination total={totalPages} color='default' page={pageNumber} variant='bordered' onChange={setPage}></Pagination>
//                 <div className='flex flex-row gap-1 items-center'>
//                     page size:{[3, 6, 12].map((size) => (
//                         <div key={size}
//                             onClick={() => setPageSize(size)} className={clsx('page-size-box', { 'bg-foreground text-white hover:bg-foreground hover:text-white': pageSize === size })}>
//                             {size}
//                         </div>
//                     ))}
//                 </div>
...
import React, { useEffect } from "react";
import clsx from "clsx";
import usePaginationStore from "../../app/hooks/usePaginationStore";
